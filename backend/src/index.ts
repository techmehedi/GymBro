...
app.use('*', cors({
  origin: (origin) => {
    ...
    // Allow network IPs for development (192.168.x.x with any port)
    const networkIpRegex = /^https?:\/\/192\.168\.\d+\.\d+:\d+$/;
    const expoNetworkRegex = /^exp:\/\/192\.168\.\d+\.\d+:\d+$/;
    
    if (networkIpRegex.test(origin) || expoNetworkRegex.test(origin)) return origin;
